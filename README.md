...
Le contrôle de qui est autorisé reste entièrement manuel (approbation
explicite requise). Deux façons de générer le jeton :
...
Transmettez le jeton en clair (non le hash) à l'opérateur de l'instance : c'est
ce qu'il devra envoyer dans le header `Authorization`. Une instance sans champ
`status` est traitée comme `"active"` (compatibilité avec les entrées ajoutées
avant l'introduction du flux d'approbation ci-dessous).
...
3. Vous consultez les demandes en attente (nécessite `ADMIN_SECRET`, voir
   `.env.example`) :
...
Les routes `/admin/*` acceptent les appels cross-origin (CORS ouvert,
`Access-Control-Allow-Origin: *`) pour permettre à un panel admin web de les
appeler directement depuis le navigateur avec `Authorization: Bearer
<ADMIN_SECRET>`. Aucun cookie/session n'est utilisé, donc pas de risque à
ouvrir l'origine.
...
publiée sur `ghcr.io/zlimteck/ebookrequest-apns-relay:latest`.

### Variables à définir avant de lancer le stack

Créer un fichier `.env` à côté de `docker-compose.yml` avec au minimum :

```bash
APNS_KEY_P8=      # contenu de la clé .p8, avec \n littéraux si sur une seule ligne
APNS_KEY_ID=
APNS_TEAM_ID=
ADMIN_SECRET=     # voir génération plus haut
NOTIFY_WEBHOOK_URL=  # optionnel
```

`instances.json` doit aussi exister dans ce dossier (copié depuis
`instances.example.json` puis rempli, voir plus haut). Il n'est jamais
versionné dans Git (`.gitignore`) et le service y écrit (approbations,
demandes en attente).

### Lancer / mettre à jour le stack
